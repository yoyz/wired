...
string akaiGetType(int type) 
{ 
  string s;
  int	i;

  for (i = 0; (kAkaiTypes[i].typeVal != 0) && (kAkaiTypes[i].typeVal != type); i++) ;
  if (kAkaiTypes[i].typeVal == type)
...
void	FileLoader::ListAkaiCD(wxTreeItemId root)
{
  if (img == NULL)
  {
    /*
       ...
       return;
       */
    img = new akaiImage(filename->GetValue().c_str());
  }
  ...
  if (tid->GetPath() == "/")
  {
    ...
    for (vector<akaiPartition *>::iterator i = parts.begin(); i != parts.end(); i++)
    {
      /*
      char a = 64 + part;
      ...
      */
      ...
      name += (*i)->getName();
      folder->AppendItem(root, name.c_str(), 0, -1, new TreeItemData(name));
    }
  }
  else
  {
    if (tid->GetPath().substr(0, 10) == "Partition ")
    {
      ...
      if ((part - 1 > 0) && (part - 1 < img->getPartitions().size()))
      {
        akaiPartition *partition = img->getPartitions()[part - 1];
        vector<akaiVolume *> vols = partition->getVolumes();
        for (vector<akaiVolume *>::iterator i = vols.begin(); i != vols.end(); i++)
          folder->AppendItem(root, (*i)->getName().c_str(), 0, -1, new TreeItemData((*i)->getPath()));
      }
    }
  }
}
  ...
  if (p.substr(0, 10) == "Partition ")
  {
    ...
    if ((part - 1 > 0) && (part - 1 < img->getPartitions().size()))
    {
      ...
      for (vector<akaiVolume *>::iterator i = vols.begin(); i != vols.end(); i++)
      {
        if ((*i)->getName() == path)
        {
          vector<akaiFile *> volfiles = (*i)->getFiles();
          int num = 0;
          for (vector<akaiFile *>::iterator j = volfiles.begin(); j != volfiles.end(); j++)
          {
            long item = files->InsertItem(num++, (*j)->getName().c_str());
            files->SetItem(item, 1, FormatSize((*j)->getSize()));
            files->SetItem(item, 2, akaiGetType((*j)->getType()).c_str());
            if (((*j)->getType() == 115) || ((*j)->getType() == 243))
