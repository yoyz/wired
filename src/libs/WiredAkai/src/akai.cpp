...
{
  return part[0]; //(toupper(part[0]) - 64);
}
...
t_akaiProgram	*akaiOpenProg(char *buffer, int size)
{
  ...
  new_prog->keygrp = 0;
  for (int i = 0; i < new_prog->nkeygrps; i++)
  {
...
t_akaiSample		*akaiGetSample(int fd, long ofs, long size)
{
  ...
  read(fd, buffer, size);
  akaiToAscii((char*)(buffer + 3), 12);
  strcpy(sample->name, (char*)(buffer + 3));
...
t_akaiProgram	*akaiLoadProgram(char *dev, int partition, char *subdir, char *name)
{
  ...
  prog = akaiOpenProg((char*)buffer, rec->size);
