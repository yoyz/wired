...
last_revision=$( svn info ${url_svn_tags}/${last_tag} | grep "Last Changed Rev" | grep -o -e "[0-9]\+$" );

#On récupère le numéro de la revision courante
echo "Getting current revision..."
current_revision=$( svn info ${url_svn}/"trunk" | grep "Last Changed Rev" | grep -o -e "[0-9]\+$" );
...
svn log --xml -rHEAD:${last_revision} ${url_svn_trunk} | grep -v "^$" | xsltproc ./transform.xsl - | grep -v "<?xml" >> ${output_file}
