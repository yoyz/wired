  ...
  : Plugin(startinfo, initinfo), Bypass(false)
{
  ...
  CReverbBtn = 
    new DownButton(this, Reverb_C, wxPoint(102, 45),
		   wxSize(c_rev_off->GetWidth(), c_rev_off->GetHeight()), 
		   c_rev_off, c_rev_on);
  AReverbBtn->SetOn();
  SelrevKnob = new StaticPosKnob(this, Reverb_Select, 3, imgs, 15, 0, 2, 0, 
				 wxPoint(70, 7), wxDefaultSize);
...
void ReverbPlugin::Load(WiredPluginData& Datas)
{
  ...
  ReverbMutex.Lock();
  buffer = strdup(Datas.LoadValue(wxString(STR_REVERB_SELECTED)));
  if (buffer != NULL)
    {
      rev_sel = atoi(buffer);
  ...
  free(buffer);
  buffer = strdup(Datas.LoadValue(wxString(STR_MIX_LEVEL)));
  if (buffer != NULL)
    {
  ...
  free(buffer);
  buffer = strdup(Datas.LoadValue(wxString(STR_DECAY)));
  if (buffer != NULL)
    {
...
void ReverbPlugin::Save(WiredPluginData& Datas)
{
  ...
  oss << rev_sel;
  Datas.SaveValue(wxString(STR_REVERB_SELECTED), (char *)oss.str().c_str());
  oss.seekp(ios_base::beg);
  oss << (MixKnob->GetValue() / EFFECT_MIX);
  Datas.SaveValue(wxString(STR_MIX_LEVEL), (char *)oss.str().c_str());
  oss.seekp(ios_base::beg);
  oss << DecayKnob->GetValue();
  Datas.SaveValue(wxString(STR_DECAY), (char *)oss.str().c_str());
  oss.seekp(ios_base::beg);
}
